...
async function testJoinProject() {
  try {
    ...
    const project = await Project.findOne({ invitationCode: { $exists: true } });
    ...
    console.log('Adding test member:', { userId: testUserId, email: testEmail });
    ...
    console.log('Member details:', project.members.map(m => ({
    ...
    console.log('Verification - Member details:', verifyProject.members.map(m => ({
      userId: m.userId?.toString(),
      email: m.email,
      role: m.role,
      status: m.status
